...
public class DynamicFormServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
        ...
        System.out.println("Received JSON Data: " + jsonBuffer.toString());
        ...
        String formName = jsonObject.optString("form_name", null); 
        String columnsJson = jsonObject.optString("columns", null); 

        if (formName == null || columnsJson == null) {
            response.getWriter().write("Error: Missing form name or columns data.");
            return;
        }
        ...
        System.out.println("Columns JSON: " + columns);
        ...
        for (int i = 0; i < columns.length(); i++) {
            ...
            String constraint = column.optString("constraint", null);
            String checkCondition = column.optString("check", null);

            queryBuilder.append(header).append(" ").append(type);

         
            if (constraint != null && !constraint.isEmpty()) {
                queryBuilder.append(" ").append(constraint);
            }
            ...
            if (checkCondition != null) {
                switch (checkCondition) {
                    case "email":
                        queryBuilder.append(" CHECK (").append(header).append(" ~* '^[a-z0-9._%+-]+@[a-z0-9.-]+\\.[a-z]{2,}$')");
                        break;
                    case "age":
                        queryBuilder.append(" CHECK (").append(header).append(" >= 18)");
                        break;
                    case "mobile":
                        queryBuilder.append(" CHECK (").append(header).append("::TEXT ~'\\d{10}$')");
                        break;
                    case "ipaddress":
                        queryBuilder.append(" CHECK (").append(header).append(" ~ '^((25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)\\.){3}(25[0-5]|2[0-4][0-9]|[01]?[0-9][0-9]?)$')");
                        break;
                    case "url":
                        queryBuilder.append(" CHECK (").append(header).append(" ~ '^https?://[a-zA-Z0-9-]+(\\.[a-zA-Z0-9-]+)+(/[^\\s]*)?$')");
                        break;
                    default:
                        break;
                }
                                ...
                                if (deptIds != null && !deptIds.isEmpty()) {
                                    ...
                                    for (String deptIdStr : ids) {
                                        ...
                                        if (deptResult.next()) {
                                            ...
                                            String updatedForms = (existingForms.isEmpty() || existingForms.equals("0") )? String.valueOf(formId) : existingForms + "," + formId;
